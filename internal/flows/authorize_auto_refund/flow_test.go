...
func Test_Authorize_AutomaticCapture_Refund_Success(t *testing.T) {
	log.Println("=================================================")
	log.Println("Testing Authorize with automatic capture, the payment will be captured automatically and will be refunded...")
	log.Println("=================================================")
	...
	log.Println("The payment must be in automatic capture mode")
	...
	assert.Equal(t, "AUTOMATIC", payment.Capture.Mode, "expected capture mode to be AUTOMATIC")
	assert.Equal(t, "AUTHORIZED", payment.Status, "expected payment status to be AUTHORIZED")

	log.Println("=================================================")
	log.Println("Initializing refund...")
	log.Println("=================================================")

	refundResponse, err := authorizationProvider.Refund(context.Background(), akuaClient, authorization.PaymentID)
	...
	log.Println("=================================================")
	log.Println("Refund Response:")
	log.Println("Refund ID: ", refundResponse.PaymentId)
	log.Println("Refund Status: ", refundResponse.Transaction.Status)
	log.Println("Refund Amount: ", refundResponse.Transaction.Amount)
	log.Println("=================================================")

	lastPaymentState, err := paymentsProvider.GetPaymentById(context.Background(), akuaClient, refundResponse.PaymentId)
	...
	log.Println("In this case we need to have the refund transaction in the payment transactions including authorization and refund")
	...
	// Validate if the payment was refunded successfully

	assert.Equal(t, "REFUNDED", lastPaymentState.Status, "expected payment status to be REFUNDED")
}
