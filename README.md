...
All flows are implemented as Go tests. Use the following commands to run specific flows:
...
To run a specific test function within a flow:

```bash
go test -v ./internal/flows/authorize_auto_capture -run Test_Authorize_AutoCapture_Success
```

### Verbose Output

The `-v` flag enables verbose output, which shows:

- Test execution progress
- Detailed logging from each phase
